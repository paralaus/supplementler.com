  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'update':
        ...
        $entry_street_address_2 = tep_db_prepare_input($HTTP_POST_VARS['entry_street_address_2']);
        // Second Address Field mod:
        ...
        if($_POST['customers_password']!=''){
            $pass=tep_db_prepare_input($_POST['customers_password']);
        }
      ...
      if ($error == false) {
        ...
        $sql_data_array = array('customers_firstname' => $customers_firstname,
                                ...
                                'customers_password' => tep_encrypt_password($pass),
                                'guest_account'=>'0',
      ...
      case 'deleteconfirm':
        ...
        break;
      default:
        $customers_query = tep_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_company, a.entry_street_address, a.entry_street_address_2, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on (c.customers_default_address_id = a.address_book_id) where a.customers_id = c.customers_id and c.customers_id = '" . (int)$HTTP_GET_VARS['cID'] . "'");
...
<?php } ?>
  ...
  if ((document.getElementById('customers_password').value != "")&&(document.getElementById('customers_password').value !=document.getElementById('customers_cpassword').value)) {
    alert("Vermiþ olduðunuz þifreler uyumlu deðil!");
    return false;
  }
...
<?php
  if ($error == true) {
    ...
    echo tep_draw_input_field('entry_street_address_2', $cInfo->entry_street_address_2, 'maxlength="64"', true);
  }
?></td>
          </tr>
<!-- :Second Address Field mod -->
...
?>
          <tr>
            ...
            <td class="main">
<?php
       echo tep_draw_input_field('entry_state', $cInfo->entry_state);
...
<input type="hidden" name="country" value="215">
      ...
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
            <td class="main"><?php echo 'Parola:'; ?></td>
            <td class="main">
                <input type="text" name="customers_password" id="customers_password">
            </td>
          </tr>
          <tr>
            <td class="main"><?php echo 'Parola Onayı:'; ?></td>
            <td class="main">
                <input type="text" name="customers_cpassword" id="customers_cpassword">
            </td>
          </tr>
        </table></td>
      </tr>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_TOTAL_ORDER; ?></td>
...
<?php
    ...
    while ($customers = tep_db_fetch_array($customers_query)) {
	 ...
	 // BOF - JUNG GESTALTEN - 27.11.2008 - KUNDENUMSÄTZE
		$umsatz_query = tep_db_query("select sum((op.final_price+(op.final_price*op.products_tax/100))*op.products_quantity) as ordersum from " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_ORDERS . " o where '".$customers['customers_id']."' = o.customers_id and o.orders_id = op.orders_id");
		$umsatz = tep_db_fetch_array($umsatz_query);
...
?>
<?php
$orders_total_query = tep_db_query("select o.customers_id, ot.text as order_total from " .TABLE_ORDERS . " o left join " .TABLE_ORDERS_TOTAL . " ot on (o.orders_id = ot.orders_id) left join " .TABLE_ORDERS_STATUS . " s on (o.orders_status = s.orders_status_id) where o.customers_id = '" . (int)$customers['customers_id'] . "' and o.orders_status = s.orders_status_id and ot.class = 'ot_total' ");
?>
               ...
               <?php if ($umsatz['ordersum'] !='') { ?>
                <td class="dataTableContent" align="right"><?php if ($umsatz['ordersum']>0) { echo $currencies->format($umsatz['ordersum']);} ?></td>
                <?php } else { ?>
